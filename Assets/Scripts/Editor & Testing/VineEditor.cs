...
public class VineEditor : MonoBehaviour {
    ...
    [ContextMenu("Add to Nodes' children")]
    void addObjectToChildren()
    {
        ...
        foreach(GameObject link in m_listedLinks)
        {
            foreach (GameObject child in addToChildren)
            {
                if (!child) // if child is set to none, skip this one
                    continue;

                newInstance = Instantiate(child);
                ...
                if (positionMode == PositionMode.keepLocalPosition)
                    newInstance.transform.localPosition = child.transform.localPosition;

                else if (positionMode == PositionMode.setToOrigin)
                    newInstance.transform.localPosition = Vector3.zero;

                if (scaleMode == ScaleMode.keepLocalScale)
                    newInstance.transform.localScale = child.transform.localScale;

                if (rotationMode == RotationMode.keepLocalRotation)
                    newInstance.transform.localRotation = child.transform.localRotation;
            }
        }
    }

    [ContextMenu("Remove Nodes' children")]
    void removeChildren()
    {
        getLinks();
        foreach (GameObject link in m_listedLinks)
